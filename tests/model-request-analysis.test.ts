...
describe('模型请求分析展示模型', () => {
  ...
  it('分析页、组成图、轨迹台账和响应分段共用角色色', () => {
    ...
    expect(styles).toContain('.chatluna-studio-model-analysis-card.is-tool-call { --chatluna-studio-role: var(--chatluna-studio-role-tool-interaction); }')
    expect(styles).toContain('.chatluna-studio-model-analysis-card.is-tool-result { --chatluna-studio-role: var(--chatluna-studio-role-tool-interaction); }')
  ...
  it('history_new 和 history_last 默认渲染消息预览，并保留原始 XML 切换', () => {
    ...
    // 引用块的标题与被引正文必须是两个块级兄弟节点：合成行内兄弟时它们会连成一句。
    expect(preview).toContain('class: \'chatluna-studio-model-history-quote\'')
    expect(preview).toContain("class: 'chatluna-studio-model-history-quote-head'")
    expect(preview).toContain("class: 'chatluna-studio-model-history-quote-title'")
    expect(preview).toContain("class: 'chatluna-studio-model-history-quote-content'")
    expect(preview).toContain('h(IconCornerUpLeft,')
    expect(preview).not.toContain('chatluna-studio-message-quote')
    ...
    expect(styles).toMatch(/\.chatluna-studio-model-history-line \{[^}]*flex-direction: column;[^}]*width: var\(--chatluna-studio-model-history-line-width\);/s)
    expect(styles).toMatch(/\.chatluna-studio-model-history-message \{[^}]*--chatluna-studio-model-history-line-width: min\(78%, 760px\);/s)
    ...
    expect(styles).toMatch(/\n\.chatluna-studio-model-history-quote \{[^}]*display: grid;[^}]*max-width: var\(--chatluna-studio-model-history-line-width\);[^}]*padding: [^}]*border-left: 3px solid var\(--chatluna-studio-model-history-quote-accent\);[^}]*background: color-mix\(in srgb, var\(--chatluna-studio-model-history-quote-accent\) \d+%, var\(--chatluna-studio-surface\)\);/s)
    expect(styles).toMatch(/\.chatluna-studio-model-history-quote-head \{[^}]*display: flex;/s)
    expect(styles).toMatch(/\.chatluna-studio-model-history-message\.is-bot \.chatluna-studio-model-history-quote-head \{[^}]*flex-direction: row-reverse;/s)
    expect(styles).toMatch(/\.chatluna-studio-model-history-quote-content \{[^}]*overflow: visible;[^}]*overflow-wrap: anywhere;[^}]*text-overflow: clip;[^}]*white-space: pre-wrap;/s)
    // 嵌套引用只保留强调条：层层叠底色会糊成一片。
    expect(styles).toMatch(/\.chatluna-studio-model-history-quote \.chatluna-studio-model-history-quote \{[^}]*background: transparent;/s)
    expect(styles).not.toContain('chatluna-studio-message-quote')
  ...
  it('四处工具载荷默认按结构显示，解析不出结构时退回原文', () => {
    ...
    expect(view.match(/class="chatluna-studio-model-analysis-tool-payload"/g)).toHaveLength(1)
    expect(toolCard.match(/class="chatluna-studio-model-analysis-tool-payload"/g)).toHaveLength(1)
  ...
  it('工具 JSON 与长正文共用折叠壳，超过 12 行时自动省略', () => {
    ...
    expect(view.match(/<AnalysisContentBlock[^>]*>\s*<div class="chatluna-studio-model-analysis-tool-payload">/gs)).toHaveLength(1)
  ...
  it('工具卡片自带折叠与原文退路，载荷样式不再嵌套标题选择器', () => {
    ...
    expect(toolCard).toContain('class="chatluna-studio-model-analysis-card is-tool-call"')
    expect(toolCard.match(/class="chatluna-studio-model-analysis-tool-card-name"/g)).toHaveLength(1)
  ...
  it('折叠长文本用渐隐遮罩并居中展开按钮，避免半透明实色透出字形', () => {
    ...
    expect(block).toContain("class: 'chatluna-studio-model-analysis-expand'")
    expect(block).toContain('展开全部（${blockProps.value.length} 字符）')
